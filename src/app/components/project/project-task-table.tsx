...
export const ProjectTaskTable = ({ taskRowObjects, arrayUpdateFuntion }: { taskRowObjects: TaskObjectTypes[], arrayUpdateFuntion: (taskObject?: TaskObjectTypes,index?:number,deleteTask?:boolean) => void }) => {

    const tableHeads = ["#", "Task Name", "Task Description", "Start Date", "End Date", " "];
    return (
                    ...
                    <tbody className="text-gray-700">
                        {taskRowObjects.map((tableRow: TaskObjectTypes, index: number) => (
                            <tr className="even:bg-blue-gray-50/50" key={tableRow.taskid}>
                                ...
                                <td className="text-left py-3 px-4">{tableRow.taskdescription}</td>
