...
export async function POST(request: Request) {
  const { staffid, projectid, taskRows } = await request.json();
  let message: string = "SUCCESS";

  console.log("taskRows", taskRows, staffid, projectid);
  try {
    await prisma.$transaction(async (tx) => {
      ...
      const isAnySelected: any = taskRows.find((t: any) => t.selected === true);
      if (!isAnySelected) {
